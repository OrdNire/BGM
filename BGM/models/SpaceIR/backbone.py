...
class ResNet101(nn.Module):
    def __init__(self, input_channels=3):
        ...
        if input_channels != 3:
            self.base_network.conv1 = nn.Conv2d(input_channels, 64, kernel_size=7, stride=2, padding=3, bias=False)
            nn.init.kaiming_normal_(self.base_network.conv1.weight, mode='fan_out', nonlinearity='relu')
...
if __name__ == '__main__':
    tensor = torch.rand(64, 3, 224, 224)
    model = ResNet50()
    x = model(tensor)
    print(x.shape)